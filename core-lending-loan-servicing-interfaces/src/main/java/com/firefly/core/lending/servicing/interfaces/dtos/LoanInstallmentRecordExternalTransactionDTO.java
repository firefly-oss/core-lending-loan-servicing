...
@NoArgsConstructor
@AllArgsConstructor
public class LoanInstallmentRecordExternalTransactionDTO {

    @JsonProperty(access = JsonProperty.Access.READ_ONLY)
    private UUID loanInstallmentRecordExternalTransactionId;

    @FilterableId
    @NotNull(message = "Loan installment record ID is required")
    ...
    @Size(max = 255, message = "Payer account number cannot exceed 255 characters")
    private String payerAccountNumber; // Customer's external account number (e.g., bank account, card last 4 digits)

    @Size(max = 255, message = "Payer name cannot exceed 255 characters")
    private String payerName; // Customer name on external account
